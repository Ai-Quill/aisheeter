...
function createBulkJob(inputData, prompt, model, specificModel) {
  ...
  // Parse compound model format (e.g., 'CHATGPT:gpt-5-mini' → provider + specificModel)
  if (model && model.indexOf(':') !== -1 && model.indexOf('MANAGED:') !== 0) {
    var parsed = parseModelSelection(model);
    if (!specificModel) {
      specificModel = parsed.specificModel;
    }
    model = parsed.provider;
    Logger.log('[createBulkJob] Parsed compound model → provider: ' + model + ', specificModel: ' + specificModel);
  }
  ...
  try {
    ...
    logJobActivity({
      ...
      message: inputData.length + ' rows queued',
      details: { model: model, rowCount: inputData.length }
    });
...
/**
 * Retry a failed or cancelled job via backend PUT endpoint.
 * Resets the job to 'queued' and triggers the worker.
 * 
 * @param {string} jobId - The job ID to retry
 * @return {Object} Retry result with new status
 */
function retryJob(jobId) {
  ...
  try {
    ...
    logJobActivity({
      ...
      action: 'retried',
      timestamp: new Date().toISOString(),
      message: 'Job retried (attempt ' + (result.retryCount || 1) + ')'
    });
    
    // Trigger worker
    triggerWorkerAsync([jobId]);
    ...
    return {
      ...
      message: result.message || 'Job queued for retry'
    };
    
  } catch (error) {
    Logger.log('[retryJob] Error: ' + error.message);
...
/**
 * Log a job activity event (created, completed, failed, retried, etc.)
 * Stored in PropertiesService for lightweight persistence.
 * 
 * @param {Object} activity - { jobId, action, timestamp, message, details? }
 */
function logJobActivity(activity) {
  try {
    var props = PropertiesService.getUserProperties();
    var log = JSON.parse(props.getProperty('jobActivityLog') || '[]');
    
    // Keep last 100 entries to stay within PropertiesService limits
    if (log.length >= 100) {
      log = log.slice(-80); // Trim to 80 when we hit 100
    }
    
    log.push({
      jobId: activity.jobId,
      action: activity.action, // 'created' | 'completed' | 'failed' | 'retried' | 'cancelled' | 'chunk_failed' | 'chunk_completed'
      timestamp: activity.timestamp || new Date().toISOString(),
      message: activity.message || '',
      details: activity.details || null
    });
    
    props.setProperty('jobActivityLog', JSON.stringify(log));
  } catch (e) {
    Logger.log('[logJobActivity] Error: ' + e.message);
  }
}

/**
 * Get the job activity log.
 * Returns recent activities with optional filtering.
 * 
 * @param {Object} [options] - { jobId?, action?, limit? }
 * @return {Array} List of activity entries
 */
function getJobActivityLog(options) {
  options = options || {};
  ...
  try {
    ...
    if (options.jobId) {
      log = log.filter(function(entry) {
        return entry.jobId === options.jobId;
      });
    }
    ...
    Logger.log('[getJobActivityLog] Error: ' + e.message);
    return [];
  }
}

/**
 * Get a summary of job activities — counts by status.
 * 
 * @return {Object} { total, completed, failed, retried, cancelled }
 */
function getJobActivitySummary() {
  try {
    var log = getJobActivityLog({ limit: 100 });
    var summary = {
      total: 0,
      completed: 0,
      failed: 0,
    ...
    log.forEach(function(entry) {
      summary.total++;
      if (entry.action === 'completed' || entry.action === 'chunk_completed') summary.completed++;
      else if (entry.action === 'failed' || entry.action === 'chunk_failed') summary.failed++;
      else if (entry.action === 'retried') summary.retried++;
      else if (entry.action === 'cancelled') summary.cancelled++;
    });
    
    return summary;
  } catch (e) {
    return { total: 0, completed: 0, failed: 0, retried: 0, cancelled: 0 };
  }
}

/**
 * Clear old job activity log entries.
 * Keeps only the most recent entries.
 * 
 * @param {number} [keepCount=50] - Number of entries to keep
 */
function clearOldJobActivities(keepCount) {
  keepCount = keepCount || 50;
  
  try {
    var props = PropertiesService.getUserProperties();
    var log = JSON.parse(props.getProperty('jobActivityLog') || '[]');
    
    if (log.length > keepCount) {
      log = log.slice(-keepCount);
      props.setProperty('jobActivityLog', JSON.stringify(log));
    }
  } catch (e) {
    Logger.log('[clearOldJobActivities] Error: ' + e.message);
  }
}
