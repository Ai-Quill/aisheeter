...
var SheetActions_Operations = (function() {
    ...
    if (config.operations && Array.isArray(config.operations)) {
      ...
      for (var i = 0; i < config.operations.length; i++) {
        ...
        try {
          ...
          Logger.log('[SheetActions_Operations] Op ' + i + ' (' + (opConfig.operation || 'unknown') + ') failed: ' + e.message);
          results.push({ operation: opConfig.operation, skipped: true, error: e.message });
        }
      }
  ...
  function _executeSingleOperation(sheet, config) {
    ...
    switch (operation) {
      ...
      case 'hideColumns':
        ...
        var numCols = config.numColumns || 1;
        var maxColHC = sheet.getMaxColumns();
        if (colIndex > maxColHC) return { hidden: 'columns', start: config.startColumn, skipped: true };
        if (colIndex + numCols - 1 > maxColHC) numCols = maxColHC - colIndex + 1;
        sheet.hideColumns(colIndex, numCols);
        return { hidden: 'columns', start: config.startColumn, count: numCols };
        
      case 'showColumns':
        var colIndex = _getColIndex(config.startColumn);
        var numCols = config.numColumns || 1;
        var maxColSC = sheet.getMaxColumns();
        if (colIndex > maxColSC) return { shown: 'columns', start: config.startColumn, skipped: true };
        if (colIndex + numCols - 1 > maxColSC) numCols = maxColSC - colIndex + 1;
        sheet.showColumns(colIndex, numCols);
        return { shown: 'columns', start: config.startColumn, count: numCols };
      ...
      case 'setColumnWidth':
        ...
        var maxColSCW = sheet.getMaxColumns();
        if (colIndex > maxColSCW) return { columnWidth: width, column: config.column || config.columns, skipped: true };
      ...
      case 'setColumnWidths':
        ...
        var maxColW = sheet.getMaxColumns();
        if (endCol > maxColW) endCol = maxColW;
        if (startCol > maxColW) return { columnWidth: config.width, skipped: true };
      ...
      case 'autoResizeColumn':
        ...
        var maxColAR = sheet.getMaxColumns();
        if (colIndex > maxColAR) return { autoResized: 'column', column: config.column, skipped: true };
      ...
      case 'autoResizeRows':
        var maxRowAR = sheet.getMaxRows();
        var startRowAR = config.startRow || 1;
        var numRowsAR = config.numRows || 1;
        if (startRowAR > maxRowAR) return { autoResized: 'rows', startRow: startRowAR, skipped: true };
        if (startRowAR + numRowsAR - 1 > maxRowAR) numRowsAR = maxRowAR - startRowAR + 1;
        sheet.autoResizeRows(startRowAR, numRowsAR);
        return { autoResized: 'rows', startRow: startRowAR, numRows: numRowsAR };
  ...
  function _autoResizeColumns(sheet, config) {
    ...
    // Clamp to actual sheet column count to avoid "out of bounds" errors
    var maxCol = sheet.getMaxColumns();
    if (endCol > maxCol) {
      Logger.log('[SheetActions_Operations] Clamping autoResize endCol from ' + endCol + ' to ' + maxCol);
      endCol = maxCol;
    }
    if (startCol > maxCol) {
      Logger.log('[SheetActions_Operations] startCol ' + startCol + ' > maxCol ' + maxCol + ', skipping autoResize');
      return { autoResized: 'columns', start: config.startColumn, end: config.endColumn || config.startColumn, skipped: true };
    }
