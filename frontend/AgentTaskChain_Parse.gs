 ...
 * Splits on chaining words — no AI, no hardcoded action detection.
 * 
...
function parseChainLocally(command) {
  ...
  // Create steps from parts — action defaults to 'process', backend will resolve
...
function fetchChainSuggestionsFromBackend(context, modelFromUI) {
  ...
  try {
    ...
    var parsedSuggChainModel = parseModelSelection(selectedSuggModel);
    var isManagedSuggChain = parsedSuggChainModel.provider === 'MANAGED';
    var managedSuggChainId = isManagedSuggChain ? parsedSuggChainModel.specificModel : null;
    var provider = isManagedSuggChain ? 'GEMINI' : parsedSuggChainModel.provider;
    ...
    if (isManagedSuggChain) {
      ...
      payload = SecureRequest.buildPayloadWithModel(provider, context, parsedSuggChainModel.specificModel);
    }
