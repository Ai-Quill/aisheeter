...
var SheetActions_Format = (function() {
        ...
        try {
          ...
          if (opFormatType && opFormatType !== 'text') {
            ...
            if (opFormatStr) {
              ...
              Logger.log('[SheetActions_Format] Skipping unknown format type "' + opFormatType + '" on ' + opRange + ' — preserving existing format');
            }
          }
      ...
      try {
        ...
        if (formatType && formatType !== 'text') {
          ...
          if (formatStr) {
            ...
            Logger.log('[SheetActions_Format] Skipping unknown format type "' + formatType + '" — preserving existing format');
          }
        }
  ...
  function _applyFormattingToRange(range, formatting, numberFormatAlreadyApplied) {
    ...
    if (formatting.banding) {
      ...
      // Remove existing banding first to avoid "already has alternating background colours" error
      try {
        var existingBandings = range.getSheet().getBandings();
        for (var bi = 0; bi < existingBandings.length; bi++) {
          var bRange = existingBandings[bi].getRange();
          // Remove if the banding overlaps with our target range
          if (_rangesOverlap(range, bRange)) {
            existingBandings[bi].remove();
            Logger.log('[SheetActions_Format] Removed existing banding from ' + bRange.getA1Notation() + ' before re-applying');
          }
        }
      } catch (clearErr) {
        Logger.log('[SheetActions_Format] Could not check/clear existing banding: ' + clearErr.message);
      }
    ...
    if (!numberFormatAlreadyApplied && (formatting.numberFormat || formatting.formatType)) {
      ...
      if (nfStr) {
        ...
        Logger.log('[SheetActions_Format] Skipping unknown format type "' + nfType + '" in formatting — preserving existing format');
      }
    }
    ...
    if (formatting.removeBanding || formatting.clearBanding) {
      try {
        ...
        for (var b = 0; b < bandings.length; b++) {
          ...
          if (_rangesOverlap(range, bandRange)) {
            bandings[b].remove();
            Logger.log('[SheetActions_Format] Removed banding from ' + bandRange.getA1Notation());
          }
        }
      } catch (e) {
   ...
   * @param {Range} r1 - First range
   * @param {Range} r2 - Second range
   * @return {boolean} True if ranges overlap
   */
  function _rangesOverlap(r1, r2) {
    var r1Top = r1.getRow(), r1Bottom = r1.getLastRow();
    var r1Left = r1.getColumn(), r1Right = r1.getLastColumn();
    var r2Top = r2.getRow(), r2Bottom = r2.getLastRow();
    var r2Left = r2.getColumn(), r2Right = r2.getLastColumn();
