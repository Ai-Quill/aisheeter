...
var SecureRequest = {
   ...
   * @param {string} [specificModel] - Specific BYOK model ID
   * @return {Object} Payload ready for the backend
   */
  buildSmartPayload: function(provider, data, isManaged, managedModelId, specificModel) {
    if (isManaged) {
      return this.buildManagedPayload(data, managedModelId);
    }
    return this.buildPayloadWithModel(provider, data, specificModel);
  }
};
