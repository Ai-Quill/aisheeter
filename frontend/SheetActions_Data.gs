...
var SheetActions_Data = (function() {
  ...
  function writeData(step) {
    ...
    // MULTI-SHEET SUPPORT: If targetSheet is specified, create or switch to that sheet
    var targetSheetName = step.targetSheet || config.targetSheet;
    if (targetSheetName) {
      var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
      var targetSheet = spreadsheet.getSheetByName(targetSheetName);
      if (!targetSheet) {
        targetSheet = spreadsheet.insertSheet(targetSheetName);
        Logger.log('[SheetActions_Data] Created new sheet: ' + targetSheetName);
      } else {
        Logger.log('[SheetActions_Data] Using existing sheet: ' + targetSheetName);
      }
      sheet = targetSheet;
    ...
    // PROTECT TEXT COLUMNS FROM DATE AUTO-CONVERSION:
    // Google Sheets auto-interprets strings like "January 2026", "Jan", "Mar 2025"
    // as dates and stores them as serial numbers. Setting format to '@' (plain text)
    // on text-heavy columns BEFORE setValues() prevents this.
    // We detect text columns by sampling data rows (skip header at index 0).
    if (numRows > 1 && numCols > 0) {
      for (var ci = 0; ci < numCols; ci++) {
        var textCount = 0;
        var numCount = 0;
        var sampleSize = Math.min(numRows - 1, 5); // sample up to 5 data rows
        for (var ri = 1; ri <= sampleSize; ri++) {
          var val = normalizedData[ri][ci];
        ...
        // If column is majority text strings, protect it as plain text
        if (textCount > numCount && textCount > 0) {
          var colRange = sheet.getRange(startRow, startCol + ci, numRows, 1);
    ...
    // Flush to ensure data is committed before any subsequent format steps
    SpreadsheetApp.flush();
    
    // NOTE: Number formatting (currency, percent, etc.) is handled by separate
    // format steps from the AI. The backend sanitizes data values before they
    // reach here, so values are already clean numbers. No frontend heuristics needed.
  ...
  /**
   * Normalize data - ensure all rows are arrays, pad to max width,
   * and coerce string values to proper types.
   * 
   * Type coercion converts "$48" → 48, "92%" → 0.92, "85K" → 85000
   * so Google Sheets treats them as numbers (for formulas, sorting, charts).
   * The backend's _sanitizeWriteData() does the same conversion authoritatively;
   * this is a defense-in-depth safety net.
   * 
   * No formatting DECISIONS are made here (e.g., "this column should be currency").
   * That's the AI's job via format tool calls.
   */
  function _normalizeData(data) {
    ...
    data.forEach(function(row, rowIdx) {
      ...
      // Type coercion: convert string values to proper JS types.
      // Header row (first row) is kept as-is — only data rows are coerced.
      if (rowIdx > 0) {
        rowArray = rowArray.map(function(cell) {
          ...
          // Percentage: "92%", "105.5%", "-3.2%" → decimal
          if (/^-?\d+\.?\d*\s*%$/.test(trimmed)) {
            var pctVal = parseFloat(trimmed) / 100;
            return pctVal;
          }
          
          // Currency with K/M/B suffix: "$85K" → 85000
          var currSuffixMatch = trimmed.match(/^\$?\s*(-?\d+\.?\d*)\s*([KkMmBb])$/);
          if (currSuffixMatch) {
            var base = parseFloat(currSuffixMatch[1]);
            var suffix = currSuffixMatch[2].toUpperCase();
            var mult = suffix === 'K' ? 1000 : suffix === 'M' ? 1000000 : 1000000000;
            return base * mult;
          }
          
          // Currency: "$42", "$85,000", "€50" → number
          var currMatch = trimmed.match(/^[$€£¥₹₩₫]\s*(-?[\d,]+\.?\d*)$/);
          if (currMatch) {
            var val = parseFloat(currMatch[1].replace(/,/g, ''));
            if (!isNaN(val)) return val;
          }
          
          // Date patterns — keep as strings
          if (/^(\d{1,4}[-\/]\d{1,2}[-\/]\d{1,4})$/.test(trimmed)) {
            return cell;
          }
          
          // Plain number with commas: "85,000" → 85000
          if (/^-?[\d,]+\.?\d*$/.test(trimmed) && trimmed.indexOf(',') !== -1) {
            var numVal = parseFloat(trimmed.replace(/,/g, ''));
            if (!isNaN(numVal)) return numVal;
          }
          
          // Plain number: "42", "-3.14", ".5"
          if (/^-?\d*\.?\d+$/.test(trimmed)) {
  ...
  // NOTE: _applyNumericFormats was removed.
  // Number formatting is now handled by:
  //   1. Backend _sanitizeWriteData() — converts "$42" → 42, "92%" → 0.92
  //   2. AI's format tool calls — applies currency/percent/number formats
  // The frontend is a pure executor with no formatting heuristics.
  ...
  function _applyTableFormatting(range) {
    ...
    try {
      // Remove any existing banding that overlaps with our range
      var allBandings = sheet.getBandings();
      for (var b = 0; b < allBandings.length; b++) {
        var bandRange = allBandings[b].getRange();
        // Check if banding overlaps with our table range
        if (bandRange.getRow() <= range.getLastRow() && bandRange.getLastRow() >= range.getRow() &&
            bandRange.getColumn() <= range.getLastColumn() && bandRange.getLastColumn() >= range.getColumn()) {
