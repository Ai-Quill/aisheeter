...
/**
 * Parse a compound model selection string into provider and specificModel
 * Supports formats:
 *   'CHATGPT:gpt-5-mini' → { provider: 'CHATGPT', specificModel: 'gpt-5-mini' }
 *   'CHATGPT'             → { provider: 'CHATGPT', specificModel: null }
 *   'MANAGED:gpt-5-mini'  → { provider: 'MANAGED', specificModel: 'gpt-5-mini' }
 * @param {string} modelStr - Model selection string
 * @return {{ provider: string, specificModel: string|null }}
 */
function parseModelSelection(modelStr) {
  if (!modelStr) return { provider: 'GEMINI', specificModel: null };
  
  // Managed models: 'MANAGED:model-id'
  if (modelStr.indexOf('MANAGED:') === 0) {
    return { provider: 'MANAGED', specificModel: modelStr.replace('MANAGED:', '') };
  }
  
  // Compound format: 'PROVIDER:model-id'
  var colonIdx = modelStr.indexOf(':');
  if (colonIdx !== -1) {
    return {
      provider: modelStr.substring(0, colonIdx),
      specificModel: modelStr.substring(colonIdx + 1)
    };
  }
  
  // Legacy format: just provider name
  return { provider: modelStr, specificModel: null };
}

/**
 * Save agent model preference
 * @param {string} model - Model ID in compound format 'PROVIDER:modelId' (e.g., 'CHATGPT:gpt-5-mini')
 *                          Also accepts legacy format 'CHATGPT' or managed 'MANAGED:xxx'
 */
function setAgentModel(model) {
  var parsed = parseModelSelection(model);
  var validProviders = ['CHATGPT', 'CLAUDE', 'GROQ', 'GEMINI', 'MANAGED'];
  
  if (validProviders.indexOf(parsed.provider) === -1) {
...
/**
 * Get agent model preference (raw stored value)
 * May return compound format like 'CHATGPT:gpt-5-mini' or legacy 'GEMINI'
 * @return {string} Current model selection
 */
...
/**
 * Get the provider name from the current agent model selection
 * @return {string} Provider name (CHATGPT, CLAUDE, GROQ, GEMINI)
 */
function getAgentProvider() {
  return parseModelSelection(getAgentModel()).provider;
}

/**
 * Get the specific model ID from the current agent model selection
 * @return {string|null} Specific model ID (e.g., 'gpt-5-mini') or null for provider default
 */
function getAgentSpecificModel() {
  return parseModelSelection(getAgentModel()).specificModel;
}
...
function executeAgentPlan(plan) {
  ...
  var modelSelection = plan.model || getAgentModel();
  var parsed = parseModelSelection(modelSelection);
  var model = parsed.provider;
  var specificModel = parsed.specificModel || plan.specificModel || null;
  ...
  if (isMultiAspect) {
    ...
    try {
      var job = createBulkJob(inputData, plan.prompt, model, specificModel);
    ...
    for (var idx = 0; idx < plan.outputColumns.length; idx++) {
      ...
      try {
        ...
        var job = createBulkJob(inputData, colPrompt, model, specificModel);
 ...
 * Returns ALL individual models per provider (not just one per provider),
 * so users can pick exact models like GPT-5 vs GPT-5 Mini vs o3.
 * 
 * @return {Array} Models with pricing info, grouped by provider
 */
function getAgentModels() {
  // Provider metadata for icons and ordering
  var providerMeta = {
    GEMINI:  { icon: '🌐', label: 'Google Gemini',    order: 1 },
    GROQ:    { icon: '⚡', label: 'Groq',             order: 2 },
    CHATGPT: { icon: '🧠', label: 'OpenAI',           order: 3 },
    CLAUDE:  { icon: '🤖', label: 'Anthropic Claude', order: 4 }
  };
  
  // Default/recommended model per provider (cheapest good option)
  var defaultModels = {
  ...
  // Fallback model list (if backend fetch fails)
  var FALLBACK_MODELS = [
    // Gemini
    { name: 'gemini-2.5-flash',                display_name: 'Gemini 2.5 Flash',               llm: 'GEMINI',  credit_price_per_token: 0.0000003 },
    { name: 'gemini-2.5-pro',                  display_name: 'Gemini 2.5 Pro',                 llm: 'GEMINI',  credit_price_per_token: 0.00000125 },
    { name: 'gemini-2.5-flash-lite',           display_name: 'Gemini 2.5 Flash-Lite',          llm: 'GEMINI',  credit_price_per_token: 0.0000001 },
    { name: 'gemini-3.1-pro-preview',          display_name: 'Gemini 3.1 Pro (Preview)',        llm: 'GEMINI',  credit_price_per_token: 0.000002 },
    { name: 'gemini-3-flash-preview',          display_name: 'Gemini 3 Flash (Preview)',        llm: 'GEMINI',  credit_price_per_token: 0.0000005 },
    { name: 'gemini-3.1-flash-lite-preview',   display_name: 'Gemini 3.1 Flash-Lite (Preview)', llm: 'GEMINI',  credit_price_per_token: 0.00000025 },
    // Groq
    { name: 'meta-llama/llama-4-scout-17b-16e-instruct', display_name: 'Llama 4 Scout',       llm: 'GROQ', credit_price_per_token: 0.00000011 },
    { name: 'openai/gpt-oss-120b',             display_name: 'GPT-OSS 120B',                   llm: 'GROQ', credit_price_per_token: 0.00000015 },
    { name: 'openai/gpt-oss-20b',              display_name: 'GPT-OSS 20B',                    llm: 'GROQ', credit_price_per_token: 0.000000075 },
    { name: 'qwen/qwen3-32b',                  display_name: 'Qwen3 32B',                      llm: 'GROQ', credit_price_per_token: 0.00000029 },
    { name: 'llama-3.3-70b-versatile',         display_name: 'Llama 3.3 70B',                  llm: 'GROQ', credit_price_per_token: 0.00000059 },
    { name: 'llama-3.1-8b-instant',            display_name: 'Llama 3.1 8B',                   llm: 'GROQ', credit_price_per_token: 0.00000005 },
    // OpenAI
    { name: 'gpt-5.4-nano',  display_name: 'GPT-5.4 Nano',  llm: 'CHATGPT', credit_price_per_token: 0.0000002 },
    { name: 'gpt-5.4-mini',  display_name: 'GPT-5.4 Mini',  llm: 'CHATGPT', credit_price_per_token: 0.00000075 },
    { name: 'gpt-5.4',       display_name: 'GPT-5.4',       llm: 'CHATGPT', credit_price_per_token: 0.0000025 },
    { name: 'gpt-5-mini',    display_name: 'GPT-5 Mini',    llm: 'CHATGPT', credit_price_per_token: 0.00000025 },
    { name: 'gpt-5',         display_name: 'GPT-5',         llm: 'CHATGPT', credit_price_per_token: 0.00000125 },
    { name: 'o4-mini',       display_name: 'o4 Mini',       llm: 'CHATGPT', credit_price_per_token: 0.0000011 },
    { name: 'o3-mini',       display_name: 'o3 Mini',       llm: 'CHATGPT', credit_price_per_token: 0.0000011 },
    { name: 'o3',            display_name: 'o3',            llm: 'CHATGPT', credit_price_per_token: 0.000002 },
    // Claude
    { name: 'claude-haiku-4-5-20251001',  display_name: 'Claude Haiku 4.5',   llm: 'CLAUDE', credit_price_per_token: 0.000001 },
    { name: 'claude-sonnet-4-6',          display_name: 'Claude Sonnet 4.6',  llm: 'CLAUDE', credit_price_per_token: 0.000003 },
    { name: 'claude-opus-4-6',            display_name: 'Claude Opus 4.6',    llm: 'CLAUDE', credit_price_per_token: 0.000005 }
  ];
  ...
  try {
    ...
    try {
      ...
      console.log('Could not fetch models from backend, using fallbacks');
    }
    ...
    // Build model list — one entry per individual model
    var availableModels = [];
    ...
    allModels.forEach(function(model) {
      ...
      // Skip STRATICO — handled separately
      if (provider === 'STRATICO') return;
      
      var meta = providerMeta[provider] || { icon: '⚙️', label: provider, order: 99 };
      
      // Compute price per MTok from credit_price_per_token
      var pricePerMTok = (model.credit_price_per_token || 0) * 1000000;
      var priceStr = '$' + pricePerMTok.toFixed(2) + '/MTok';
      
      // Check if this is the default/recommended model for the provider
      var isDefault = defaultModels[provider] === model.name;
      
      availableModels.push({
        id: provider + ':' + model.name,  // Compound: "CHATGPT:gpt-5-mini"
        provider: provider,
        ...
        price: priceStr,
        order: meta.order,
        priceValue: pricePerMTok,         // For sorting within provider
        recommended: isDefault
      });
    });
    
    // Sort: by provider order first, then by price within each provider
    availableModels.sort(function(a, b) {
      if (a.order !== b.order) return a.order - b.order;
      return a.priceValue - b.priceValue;
    });
    
    // If no models configured, return a message
    ...
    // Minimal fallback
    return [
      { id: 'GEMINI:gemini-2.5-flash', provider: 'GEMINI', name: 'Gemini 2.5 Flash', icon: '🌐', price: '$0.15/MTok', recommended: true },
      { id: 'CHATGPT:gpt-5-mini',      provider: 'CHATGPT', name: 'GPT-5 Mini',      icon: '🧠', price: '$0.25/MTok', recommended: false },
      { id: 'GROQ:meta-llama/llama-4-scout-17b-16e-instruct', provider: 'GROQ', name: 'Llama 4 Scout', icon: '⚡', price: '$0.11/MTok', recommended: false },
      { id: 'CLAUDE:claude-haiku-4-5-20251001', provider: 'CLAUDE', name: 'Claude Haiku 4.5', icon: '🤖', price: '$1.00/MTok', recommended: false }
    ];
  }
}
...
function parseCommandWithAI(command, context) {
  ...
  // Get user's selected model — may be BYOK (e.g., 'CHATGPT:gpt-5-mini') or managed (e.g., 'MANAGED:gpt-5-mini')
  var selectedModel = context.provider || getAgentModel() || 'GEMINI';
  var parsed = parseModelSelection(selectedModel);
  var isManaged = parsed.provider === 'MANAGED';
  var managedModelId = isManaged ? parsed.specificModel : null;
  var provider = isManaged ? 'GEMINI' : parsed.provider; // Provider is irrelevant for managed, but needed for payload shape
...
function fetchClassificationOptions(columnHeaders) {
  ...
  var parsedClassModel = parseModelSelection(selectedModel);
  var isManagedModel = parsedClassModel.provider === 'MANAGED';
  var managedId = isManagedModel ? parsedClassModel.specificModel : null;
  var provider = isManagedModel ? 'GEMINI' : parsedClassModel.provider;
  ...
  if (isManagedModel) {
    ...
    payload = SecureRequest.buildPayloadWithModel(provider, { columns: columnHeaders }, parsedClassModel.specificModel);
  }
...
function estimateAgentCost(rowCount, taskType, model, sampleText) {
  ...
  try {
    var estimateModel = model || getAgentModel();
    var parsedEstimate = parseModelSelection(estimateModel);
    var response = ApiClient.post('AGENT_ESTIMATE', {
      rowCount: rowCount,
      taskType: taskType || 'custom',
      model: parsedEstimate.provider,
      specificModel: parsedEstimate.specificModel,
    ...
    if (lastCol > 0 && lastRow > 1) {
      for (var col = 1; col <= Math.min(lastCol, 26); col++) { // Limit to A-Z
        ...
        if (dataInfo.hasData) {
          ...
          // Get ALL column values for AI context (full data visibility)
          // Capped at 200 rows to avoid payload size issues on large sheets
          var capEnd = Math.min(dataInfo.endRow, dataInfo.startRow + 199);
          sampleData[colLetter] = getColumnData(sheet, col, dataInfo.startRow, capEnd);
        }
      }
    }
...
/**
 * Get ALL values from a column (for full data visibility)
 * @param {Sheet} sheet - The sheet to read
 * @param {number} colNum - Column number (1-indexed)
 * @param {number} startRow - First data row
 * @param {number} endRow - Last data row
 * @return {Array<string>} All non-empty values in the column
 */
function getColumnData(sheet, colNum, startRow, endRow) {
  try {
...
function callSuggestionsAPI(requestBody) {
  try {
    ...
    var parsedSuggModel = parseModelSelection(selectedModel);
    var isManagedSugg = parsedSuggModel.provider === 'MANAGED';
    var managedSuggId = isManagedSugg ? parsedSuggModel.specificModel : null;
    var provider = isManagedSugg ? 'GEMINI' : parsedSuggModel.provider;
    ...
    if (isManagedSugg) {
      ...
      payload = SecureRequest.buildPayloadWithModel(provider, requestBody, parsedSuggModel.specificModel);
    }
    
    Logger.log('[Suggestions] Calling API with provider: ' + provider + ', model: ' + (parsedSuggModel.specificModel || 'default'));
