...
var SheetActions_Utils = (function() {
  ...
  function getNumberFormat(type, options) {
    options = options || {};
    // Default decimals: 0 for percent (98% not 98.00%), 2 for everything else
    var isPercentType = (type === 'percent' || type === 'percentage');
    var defaultDecimals = isPercentType ? 0 : 2;
    var decimals = options.decimals !== undefined ? options.decimals : defaultDecimals;
    ...
    // Unknown format type — return null to preserve existing cell formatting.
    // Previously defaulted to '#,##0.##' which silently overwrote currency ($) formats.
    Logger.log('[SheetActions_Utils] Unknown format type "' + type + '", preserving existing format');
    return null;
  }
