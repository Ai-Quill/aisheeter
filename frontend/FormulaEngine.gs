...
var FormulaEngine = (function() {
  ...
  function applyFormulaToRange(formulaTemplate, outputColumn, startRow, endRow) {
    try {
      ...
      var normalizedTemplate = (typeof _normalizeFormulaSyntax === 'function') ? _normalizeFormulaSyntax(formulaTemplate) : formulaTemplate;
      for (var row = startRow; row <= endRow; row++) {
        var formula = normalizedTemplate.replace(/\{ROW\}/g, row);
